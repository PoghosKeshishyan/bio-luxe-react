...
export function Slider({ slider, sliderIcons }) {
    ...
    return (
        <div id="home" style={{ backgroundImage: `url(${BACKEND_API_URL + slider[currentIndex].background_image})` }}>
            <div className="container">
                <div className="row">
                    <div className="content show" key={currentIndex}>
                        <h1>7 {slider[currentIndex].title}</h1>
                        <p className="descr">{slider[currentIndex].descr}</p>
                        <a href="/" className="btn">{slider[currentIndex].btn_text}</a>
                    </div>
                    <div className="img_div">
                        <img className="show" src={BACKEND_API_URL + slider[currentIndex].image} alt="slide" />
                    </div>
                    <div className="controls">
                        <div className="arrow_div" onClick={prevSlide}>
                            <img src={BACKEND_API_URL + sliderIcons.left_arrow_icon} alt="left" />
                        </div>
                        <div className="number">{String(currentIndex + 1).padStart(2, '0')}</div>
                        <div className="arrow_div" onClick={nextSlide}>
