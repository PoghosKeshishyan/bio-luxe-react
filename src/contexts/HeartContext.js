...
export function HeartProvider({ children }) {
    ...
    const heartIconRef = useRef(null); // Ավելացնում ենք heartIconRef
    ...
    useEffect(() => {
        const handleClickOutsideHeart = (event) => {
            if (
                ...
                heartIconRef.current && !heartIconRef.current.contains(event.target) // Ստուգում ենք icon-ի սեղմումը
            ) {
    ...
    return (
        <HeartContext.Provider value={{
            ...
            handleHeartIcon,
            heartSidebarRef,
            heartIconRef, // Ավելացնում ենք heartIconRef այստեղ
            favoritesList,
